...
import type { LoginLog, LoginLogInsert } from "../../types/database.schema.js";
 ...
 */
export async function logLogin(logData: Partial<LoginLogInsert>): Promise<LoginLog | null> {
	try {
		const {
			user_id,
			email,
 ...
 * @returns {Promise<Object>} 登录日志列表
 */
export async function getAllLoginLogs(options: Partial<LoginLogQueryOptions> = {}): Promise<{
	logs: LoginLog[];
	total: number;
}> {
	try {
...
export async function getLoginStats(userId = null) {
	try {
		...
		const { count: successful, error: successError } = await buildQuery(
			supabase.from("login_logs").select("id", { count: "exact", head: true }).eq("success", true),
		);
 ...
 */
export async function getLoginTrend(options: Partial<LoginLogQueryOptions> = {}) {
	try {
 ...
 */
export async function getRecentFailedAttempts(options: Partial<LoginLogQueryOptions> = {}) {
	try {
